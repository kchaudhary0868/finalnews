 ...
 */
function gutenify_render_post_list_block( $attributes ) {
	global $post;
	$args = array(
...
function gutenify_post_list( $posts, $attributes ) {
	...
	$class_name  = array( 'wp-block-gutenify-post-list', 'gutenify-section-' . $attributes['blockClientId'], 'gutenify-post-list-' . $attributes['layout'], 'gutenify--post-list--post-list-' . $list_layout );
 ...
 */
function gutenify_register_post_list_block() {
	...
	ob_start();
	include GUTENIFY_PLUGIN_DIR . 'src/blocks/post-list/block.json';
	...
	register_block_type(
		...
		array(
			...
			'render_callback' => 'gutenify_render_post_list_block',
		)
	);
}
add_action( 'init', 'gutenify_register_post_list_block' );
